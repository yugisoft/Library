...
{
    INTERFACES.OnPhoneTextListener onPhoneTextListener;

    public void  setOnPhoneTetListener(INTERFACES.OnPhoneTextListener listener)
    {
        onPhoneTextListener=listener;
    }
    ...
    private boolean selectionChanged,Listener=true;
...
//		if (text == null || text.equals("")) return;
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        ...
        try {
            if(MaskedEditText.this.getPhoneText().length() == PhoneLenght() && onPhoneTextListener!=null && Listener)
            {
                onPhoneTextListener.onPhoneText(getPhoneText());
                Listener=false;
            }
            else if
                    (
                            MaskedEditText.this.getPhoneText().toString().length() != PhoneLenght()
                    )
            {
                Listener=true;
            }
    ...
    int PhoneLenght() {
       ...
       int len=0;
        for (int i = 1; i <= getMask().length();i++)
        {
            String k =getMask().substring((i-1),i);
            switch (k)
            {
                case "#":
                    len++;
    ...
    public String getPhoneText()
    {
       try
       {
           ...
           {
               String k =getText().toString().substring((i-1),i);
               String ilk3=getText().toString().substring(0,2);
               switch (say(ilk3,"#"))
               {
                   case 2:s += getNember(k);break;
                   case 1:if(i>1) s += getNember(k);break;
                   default:if(i>2)s+=getNember(k);break;
               }
           }
    ...
    public String getPhoneTextZero()
    {
        String str =MaskedEditText.this.getText().toString().replace("(","").replace(")","").replace(" ","").replace("-","");
        return str;
    }
    int say(String s, String chr)
    {

        int i =0,index=-1;
        do
        {
            index=s.indexOf(chr,index+1);
            if(index!=-1)i++;
        }
        while(index!=-1);
        return i;
    ...
    public Range calculateRange(int start, int end) {
