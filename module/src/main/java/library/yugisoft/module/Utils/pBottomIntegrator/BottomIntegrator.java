...
public class BottomIntegrator extends LinearLayout implements ViewPager.OnPageChangeListener , View.OnClickListener{

    public BottomIntegrator(Context context) {
        this(context,null,0);
    }
    public BottomIntegrator(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs,0);
    }
    public BottomIntegrator(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void setIndex(int index) {
        ...
        BottomIntegrator.this.invalidate();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (focusedTextView != null)
            try
            {
                ...
                for (int i= 0 ; i<= indexOfChild(focusedTextView) ;i++)
                    x2 += getChildAt(i).getWidth();
                //x1+focusedTextView.getWidth()
    ...
    public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels)
    {
        ...
        float po = 0;
            for (int i = 0; i< position;i++)
                po += getChildAt(i).getWidth();

            x1 = po + (focusedTextView.getWidth() * xpositionOffset) / this.getWidth();
